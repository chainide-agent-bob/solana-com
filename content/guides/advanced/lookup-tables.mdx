...
Address Lookup Tables are only supported in versioned (v0) transactions. Please
note the following format details:

> [!NOTE] > **Address Lookup Tables require Versioned Transactions (v0):**
>
> Only versioned transactions (v0) can utilize address lookup tables. In a v0
> transaction, the first byte is a version prefix (e.g., `0x80` for v0),
> followed by a three-byte message header. The header structure is the same as
> in legacy transactions, but its offset is different. For details on the v0
> transaction format, see the
> [Versioned Transactions guide](/developers/guides/advanced/versions#current-transaction-versions).
